...
function Godwansdatalist(godwanslist) {
    ...
    for (var i = 0; i < Rbklist.length; i++) {
        ...
        if (Rbklist[i].WAREHOUSE_TYPE_ID == 1) {
            locations1.push(Rbklist[i].WAREHOUSE_TYPE_ID);
        }
        else if (Rbklist[i].WAREHOUSE_TYPE_ID == 2) {
            locations2.push(Rbklist[i].WAREHOUSE_TYPE_ID);
        }
        else if (Rbklist[i].WAREHOUSE_TYPE_ID == 3 || Rbklist[i].WAREHOUSE_TYPE_ID == 4 || Rbklist[i].WAREHOUSE_TYPE_ID == 5) {
            locations3.push(Rbklist[i].WAREHOUSE_TYPE_ID);
        }

        locations.push(haplocations);

        features.push({ position: new google.maps.LatLng(Rbklist[i].Latitude, Rbklist[i].Longitude), type: Rbklist[i].WAREHOUSE_TYPE_ID });
